...
class TalentWidget {
  constructor() {
    ...
    this.wsUrl = (s && s.dataset && s.dataset.ws) || 'ws://localhost:8765/ws/default';
  ...
  _downloadReport() { var fp=this.shadow.getElementById('fullscreen-panel'); var html='<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>对比报告</title><style>body{font-family:Inter,-apple-system,sans-serif;max-width:900px;margin:40px auto;padding:0 20px;color:#1D1D1F}h3{margin-bottom:16px}th,td{padding:8px 12px;border-bottom:0.5px solid #E5E5EA;text-align:left;font-size:13px}th{color:#86868B;text-transform:uppercase;font-size:11px}.chip{font-size:10px;border:0.5px solid #E5E5EA;border-radius:10px;padding:2px 7px;margin:2px;display:inline-block}.report-section h4{color:#86868B;font-size:12px;margin-bottom:8px}</style></head><body>'+fp.innerHTML+'</body></html>'; var b=new Blob([html],{type:'text/html;charset=utf-8'}); var u=URL.createObjectURL(b); var a=document.createElement('a'); a.href=u;a.download='对比报告.html';document.body.appendChild(a);a.click();document.body.removeChild(a);setTimeout(function(){URL.revokeObjectURL(u);},5000);this._addBotMsg('报告已生成下载'); }
}
...
'.result-card{background:var(--white);border-radius:var(--radius-sm);padding:12px;box-shadow:var(--shadow-sm);border:0.5px solid var(--border);width:100%}.result-card .card-top{display:flex;align-items:flex-start;justify-content:space-between}'+
