...
class TalentWidget {
  ...
  _downloadReport() { this._printReport(); }
  _printReport(){var fp=this.shadow.getElementById('fullscreen-panel');var isDetail=!!fp.querySelector('.report-header')&&!fp.querySelector('.cmp-table');var fname=isDetail?'人才全景履历':'人才对比报告';var css=':root{--green:#22c55e;--green-dark:#16a34a;--green-ghost:rgba(34,197,94,0.08);--text:#1D1D1F;--text-secondary:#86868B;--bg:#FAFAFA;--border:#E5E5EA;--border-light:#F2F2F7;--white:#fff}*{box-sizing:border-box;margin:0;padding:0}body{font-family:Inter,-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif;max-width:1100px;margin:32px auto;padding:0 20px;color:#1D1D1F;line-height:1.7;background:#fff}'+
    ...
    '@media print{@page{size:A4 landscape;margin:12mm}body{margin:0;padding:0;max-width:none}.action-btn{display:none!important}.print-hint{display:none}}';
  var origin=window.location.origin;var inner=fp.innerHTML.replace(/src="\/widget\/avatars\//g,'src="'+origin+'/widget/avatars/');var html='<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>'+fname+'</title><style>'+css+'</style></head><body><div class="print-hint" style="background:#FFF3CD;color:#856404;padding:8px 16px;margin-bottom:16px;border-radius:6px;font-size:12px">提示：打印时请取消勾选「更多设置」&gt;「页眉和页脚」以获得整洁的 PDF</div><div style="background:#fff;padding:24px">'+inner+'</div></body></html>';
